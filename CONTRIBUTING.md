Contributing to Gamblecore
=======

We're working hard to make *gamblecore* the most powerful JavaScript library for working with gamblecoin. Our goal is to have *gamblecore* be a library that can be used by anyone interested in gamblecoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in gamblecore that any change must adhere.
...
When possible, gamblecore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Gamblecore methods. For example, calling `JSON.stringify` on a Gamblecore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'gamblecore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the gamblecore object.

DO:
```javascript
var gamblecore = require('../');
var PublicKey = gamblecore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add gamblecoin-project git@github.com:gamblecoin-project/gamblecore.git
git pull --rebase gamblecoin-project master
```
...
```
Finally go to [github.com/gamblecoin-project/gamblecore](https://github.com/gamblecoin-project/gamblecore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of gamblecore.

If you have any questions feel free to post them to
[github.com/gamblecoin-project/gamblecore/issues](https://github.com/gamblecoin-project/gamblecore/issues).
